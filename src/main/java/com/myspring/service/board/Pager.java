...
public class Pager {
	...
	private int curBlock;	// 현재 페이지 블록 갯수
	...
	// [이전]blockBegin 12 13 14 15 16 17 18 19 blockEnd [다음]
	...
	public Pager(int count, int curPage) {
		...
		setPageRange();	// #{start} and #{end}에 입력될 값 계산
	...
	public void setBlockRange() {
		...
		prevPage = (1 == curBlock) ? 1 : (curBlock - 1) * BLOCK_SCALE;
	...
	public void setPageRange() {
		// DB의 페이지 분할 where rn betwwen #{start} and #{end}에 입력될 값
	...
	public void setTotPage(int count) {
		totPage = (int)Math.ceil(count * 1.0 / PAGE_SCALE);	// Math.ceil(실수) 반올림 처리
	}
